...
kubectl create secret tls keycloak-tls --cert server.crt --key server.key
```
...
Now we can login to _Keycloak_.  But first we will need to get the password for the _keyclaok_ user.  The following command shows how.
